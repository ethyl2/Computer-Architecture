...
- Make a list of files here; write a short 3-10-word description of what each file does. Note what has been implemented, and what hasn't.

  Inside ls8 directory:

  - examples directory: contains several ready-to-use .ls8 programs that can be run with our LS-8 emulator.
    They contain lines of code. Comments are prefaced with #. Instructions and arguments are written in 8-bit binary.

  - ls8.py is what we can run to initiate an instance of our CPU class, and then call its load() and run() methods. We shouldn't have to modify this code? At least, not to begin with.

  - cpu.py contains the CPU class. We will need to complete the init(), as well as modify the load() method so that it will load a program, and add to the run() method. For additional math functionality, we will need to add to the alu() method to handle additional ops. The trace() method is already written, so we can use it when debugging.
